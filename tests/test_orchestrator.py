...
def _run(max_rounds: int = 3) -> DebateState:
...
def test_clean_draft_is_published_after_one_round(agents: dict[str, Mock]) -> None:
    ...
    assert agents["critique_draft"].call_args.kwargs == {"model": "skeptic-model", "draft": _draft(1)}
...
def test_analyst_revision_sees_the_previous_round(agents: dict[str, Mock]) -> None:
    ...
    def produce_draft(client, *, model, state):
